...
def cooling(T, nH):
    return ALPHA_B * nH * KB * T * (0.684 - 0.0416 * log(T * Z2_inv * 1e-4) + 0.54 * (T * 1e-4) ** (0.37))

def cooling_deriv(T, nH):
    return ALPHA_B * nH * KB * (0.684 - 0.0416 * (log(T * Z2_inv * 1e-4) + 1) + 0.54 * 1.37 * (T * 1e-4)**0.37)
...
def main():
    ...
    # in the log
    root, aerr, rerr = false_position(equilibrium, bracket, atol=100, rtol=1e-5, max_iters=int(1e5))
    ...
    for nH in [1e-4, 1, 1e4]:
        ...
        root, aerr, rerr = newton_raphson(func, func_deriv, x0, atol=1e-10, rtol=1e-10, max_iters=int(1e5))
