def main():
    ...
    from helperscripts.optimize import golden_section, levenberg_marquardt, quasi_newton
    ...
    xmax_gauss = golden_section(lambda x: -1 * best_model_gauss(x), 0.2, 0.6, rtol=1e-8)
    xmax_poiss = golden_section(lambda x: -1 * best_model_poiss(x), 0.2, 0.6, rtol=1e-8)
    ...
    fit_model = lambda edges, a, b, c: nhalo * binned_model(edges, Nsat, a, b, c, use_integral=False)
    ...
    for i in range(Niters):
        ...
        gauss_ax.stairs(fit_model(edges, *params_gauss) / np.diff(edges) / nhalo, edges=edges, ec="gray", alpha=0.1)
        poiss_ax.stairs(fit_model(edges, *params_poiss) / np.diff(edges) / nhalo, edges=edges, ec="gray", alpha=0.1)
    ...
    ## Statistics ##
    ################
    chi_squared = gaussian_logL(data, fit_model, sigma(edges, *avg_mc_parameters_gauss), avg_mc_parameters_gauss)
    chi_squared_reduced = chi_squared / (Nbins - 4)  # 4 degrees of freedom
    logL = poissonian_logL(data, fit_model, sigma(edges, *avg_mc_parameters_poiss), avg_mc_parameters_poiss)
    ...
    gauss_ax.stairs(hist / np.diff(edges) / nhalo, edges=edges, ec="b", label="Binned data")
    poiss_ax.stairs(hist / np.diff(edges) / nhalo, edges=edges, ec="b", label="Binned data")

    gauss_ax.stairs(fit_model(edges, *best_parameters_gauss) / np.diff(edges) / nhalo, edges=edges, ec="r", label="Best fit (gaussian)")
    poiss_ax.stairs(fit_model(edges, *best_parameters_poiss) / np.diff(edges) / nhalo, edges=edges, ec="r", label="Best fit (poissonian)")
    
    gauss_ax.stairs(fit_model(edges, *avg_mc_parameters_gauss) / np.diff(edges) / nhalo, edges=edges, ec="k", label="MC fit (gaussian)")
    poiss_ax.stairs(fit_model(edges, *avg_mc_parameters_poiss) / np.diff(edges) / nhalo, edges=edges, ec="k", label="MC fit (poissonian)")
    ...
    gauss_fig.savefig("figures/04_satellite_galaxies_gaussian_mc", bbox_inches="tight", dpi=600)
    poiss_fig.savefig("figures/04_satellite_galaxies_poissonian_mc", bbox_inches="tight", dpi=600)
